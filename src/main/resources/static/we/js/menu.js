...
layui.define('jquery', function(exports) {
	...
	var menu = {
        /**
		 * 根据请求地址获取格式化的菜单数据并动态生成侧边栏菜单
         * @param curl 获取格式化数据的地址 格式参照menu.json
         */
		getMenu: function(curl) {
			...
			$.getJSON(curl, function(res) {
				...
				that.getMenuFromData(menuList)
			});
		},
        /**
		 * 直接根据格式化好的数据动态生成菜单
         * @param data json格式的菜单数据，格式参照menu.json
         */
		getMenuFromData:function (data) {
			var that = this;
            if (!$('#side-nav').length) {
                return;
            }
